    ...
    $.widget('blueimp.fileupload', $.blueimp.fileupload, {
        ...
        options: {
            ...
            start: function (e) {
                ...
                that._transition($(this).find('.fileupload-progress')).done(
                    function () {
                        $(this).find('.progress-bar').css('background-color', '#5cb85c');
            ...
            stop: function (e) {
                ...
                that._transition($(this).find('.fileupload-progress')).done(
                    function () {
                        $(this).find('.progress-bar').css('background-color', 'rgba(0, 0, 0, 0)');
                        $(this).find('.progress')
                            .attr('aria-valuenow', '0')
                            .children().first().css('width', '0%');
                        $(this).find('.progress-extended').html('&nbsp;completed');
                        deferred.resolve();
                    }
                );
            },
            ...
            change: function (e, data) {
                ...
                if (maxFileCount < (data.files.length) + (uploadedFiles.length - 1)) {
                    alert('복호화 요청 최대 허용 파일개수를 초과하였습니다. (최대 ' + maxFileCount + '개)');
                    return false;
                }
                ...
                if (maxFileSize < tempFileSize + uploadedFileSize) {
                    alert('복호화 요청 최대 허용 크기를 초과하였습니다. (최대 ' + maxFileSize / (1024 * 1024) + 'MB)');
                    return false;
                }
                ...
                $.each(data.files, function (index, file) {
                    ...
                    if (file.name.length > 104) {
                        alert(file.name + ' 해당 파일은 파일명이 길어 인식할 수 없습니다.');
                        isMaxName = true;
                    }
        ...
        _cancelHandler: function (e) {
            ...
            if (data.abort) {
                ...
                // 파일 삭제 시 uploadedFiles 목록도 같이 삭제되도록 삭제될 index 를 가져옴
                $.each(uploadedFiles, function (idx, f) {
                ...
                // uploadedFiles 에 파일이 없다면 dropzone 을 fadein
